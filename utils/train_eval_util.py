...
def set_val_loader(args, preprocess=None):
    ...
    if args.in_dataset == "imagenet":
        ...
        val_loader = torch.utils.data.DataLoader(
            datasets.ImageFolder(os.path.join(args.root, 'imagenet100/images/val'), transform=preprocess),
            batch_size=args.batch_size, shuffle=False, **kwargs)
    elif args.in_dataset == "imagenet10":
        val_loader = torch.utils.data.DataLoader(
            datasets.ImageFolder(os.path.join(args.root, 'imagenet10/images/val'), transform=preprocess),
            batch_size=args.batch_size, shuffle=False, **kwargs)
    elif args.in_dataset == "imagenet20":
        val_loader = torch.utils.data.DataLoader(
            datasets.ImageFolder(os.path.join(args.root, 'imagenet20/images/val'), transform=preprocess),
            batch_size=args.batch_size, shuffle=False, **kwargs)
    else:
